    ...
    function ltrim(str, chars)
 {
  ...
  return (
    ...
    <div className="container my-3" style= {{color : props.mode ==='dark'?'white':'#212529'}} >
      ...
      <p>Number of words are {text.split(/\s+/).filter((element)=>  {return element.length!==0}).length} and characters are {text.length}</p>
      <p>Time to read the text will be {0.08 * text.split(/\s+/).filter((element)=>  {return element.length!==0}).length} minutes</p>
    </div>
    </>
  );
}
